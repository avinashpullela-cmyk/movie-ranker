...
function getAuth() {
  ...
  if (!keyPath) throw new Error('GOOGLE_SERVICE_ACCOUNT_KEY not set in .env');
...
export async function readMovies() {
  ...
  const parse = (rows, lang) =>
    (rows ?? []).slice(1).filter(r => r[0]?.trim()).map((r, i) => ({
      id: `${lang[0].toLowerCase()}_${i}`, name: r[0].trim(),
      year: parseInt(r[1]) || null, language: lang,
      score: 0, wins: 0, losses: 0, buchholz: 0, opponents: [],
    }));
...
export async function writeRankings(rankings) {
  ...
  await sheets.spreadsheets.values.update({
    spreadsheetId: SPREADSHEET_ID, range: 'Rankings!A1', valueInputOption: 'RAW',
    requestBody: {
      values: [
        ['Rank', 'Movie', 'Language', 'Year', 'Score', 'Wins', 'Losses', 'Games Played'],
        ...rankings.map(m => [m.rank, m.name, m.language, m.year ?? '', m.score, m.wins, m.losses, m.wins + m.losses]),
      ],
    },
  });
}
