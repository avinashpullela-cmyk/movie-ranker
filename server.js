...
function updateBuchholz(movies) {
  ...
  for (const m of movies) m.buchholz = m.opponents.reduce((sum, id) => sum + (scoreMap[id] ?? 0), 0);
}
...
app.get('/api/status', (req, res) => {
  ...
  res.json({ initialized: true, round: s.currentRound, totalMovies: s.movies.length,
    pairsInRound: s.currentPairs.length, pairsCompleted: s.currentPairs.length - pending,
    pairsRemaining: pending, totalComparisons: s.totalComparisons });
});
...
app.post('/api/init', async (req, res) => {
  ...
  try {
    ...
    if (byeId) { const m = movies.find(x => x.id === byeId); if (m) { m.score++; m.wins++; } }
...
app.get('/api/match', (req, res) => {
  ...
  res.json({
    ...
    round: s.currentRound, pairsRemaining: s.currentPairs.filter(p => !p.completed).length,
    pairsInRound: s.currentPairs.length, totalComparisons: s.totalComparisons,
  });
});
...
app.post('/api/match', (req, res) => {
  ...
  if (winnerId && winnerId !== 'skip') {
    ...
    winner.score++; winner.wins++; loser.losses++;
    ...
    pair.result = winnerId; s.totalComparisons++;
  } else {
  ...
  if (roundComplete) {
    ...
    if (byeId) { const m = s.movies.find(x => x.id === byeId); if (m) { m.score++; m.wins++; } }
    s.currentPairs = pairs;
  }
  save(s);
  res.json({ success: true, roundComplete, newRound: s.currentRound,
    pairsRemaining: s.currentPairs.filter(p => !p.completed).length, totalComparisons: s.totalComparisons });
});
...
app.post('/api/sync', async (req, res) => {
  const s = load();
  if (!s) return res.status(400).json({ error: 'Not initialized' });
  try { const rankings = getRankings(s.movies); await writeRankings(rankings); res.json({ success: true, count: rankings.length }); }
  catch (e) { res.status(500).json({ error: e.message }); }
});
...
app.listen(PORT, () => console.log(`Movie Ranker running at http://localhost:${PORT}`));
